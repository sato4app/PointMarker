...
- **実装箇所**: Canvas.jsコンストラクタで `this.dpr = window.devicePixelRatio || 1.0`
...
- `CanvasRenderer.zoomIn()` / `zoomOut()`: ズーム処理・canvasScale更新
- `CanvasRenderer.panUp()` / `panDown()` / `panLeft()` / `panRight()`: パン処理
- `CanvasRenderer.resetTransform()`: 表示リセット
- `CanvasRenderer.drawPoint()` / `drawDiamond()` / `drawSquare()`: devicePixelRatio補正付きマーカー描画
...
- **統一エラー表示**:
  - 形式エラー: 薄いピンク背景（#ffebee）+ 赤枠（#f44336）
  - 存在エラー: 赤枠のみ
  - 重複エラー: 両フィールド赤枠
...
- **二重補正システム**: devicePixelRatio補正 + canvasScale補正による正確なサイズ維持
- **適用範囲**: drawPoint、drawDiamond、drawSquare全メソッド
- **ユーザー体験向上**: どのディスプレイ環境でも一貫したマーカー表示

### 11.2 オーバーレイレイアウト固定
- **レイアウトモード変更**: サイドバーモード削除、オーバーレイモードのみ対応
...
- **ポイント削除統合**: onPointIdChangeコールバックでblur時の空白ID削除処理
