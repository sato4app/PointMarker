# PointMarker 利用者の手引
**Version 2025年11月版 v5.4（リファクタリング完了版）**

## はじめに
PointMarkerは、ハイキングマップの画像からポイント・スポット・ルートをマーキングし、座標データを管理するWebアプリケーションです。登山計画の作成や、実際に歩いたルートの記録、さらに特徴的なスポット情報の管理、複数ルートの保存・管理にも活用できます。

**v5.4の主要改善点**（v5.3からの変更）：
- **アーキテクチャの改善**：コード品質と保守性の大幅向上（約90行のコード削減）
- **Firebase連携機能**：オプションでFirestoreへのデータ保存・同期が可能に
- **複数ルート管理**：複数のルートを保存・選択・編集可能
- **処理の高速化**：統合オブジェクト検出による効率化
...
すべての処理はブラウザ内で完結し、プライバシーを保護しながら安全にご利用いただけます。Firebase連携を利用する場合は、Google認証または匿名認証で安全にデータを保存できます。
